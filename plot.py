...
def vis_saliency_kde(map, input, filename=None):
    ...
    Img = Image.fromarray(map_color).convert("RGB")
    Img = np.array(Img)
    Img = cv2.cvtColor(Img, cv2.COLOR_RGB2BGR)
    input = cv2.cvtColor(input, cv2.COLOR_RGB2BGR)
    Img = cv2.addWeighted(Img, 0.5, input, 0.5, 0)
...
def main():
    ...
    model = create_model(
        ...
        depths=args.layer,
        sr_ratios=1,
        use_conv_as_linear=args.use_conv_as_linear,
...
def plot_spike(data, filename):
    data = data.cpu().mean(dim=0).mean(dim=1).squeeze().numpy()
...
def validate(
    ...
    for index in [0, 42, 223, 929, 1146, 2048, 4826, 9547, 10086, 30851]:
        if not os.path.exists("images/" + str(index)):
            os.makedirs("images/" + str(index))
        input, target = dataset_eval.__getitem__(index)
        ...
        input_img = np.array(input)
        input.save("images/" + str(index) + "/input.png")
        input = (
            transforms.Compose(
                [
                    transforms.ToTensor(),
        ...
        for i in [0, 1, 2, 3, 4, 5, 6, 7]:
            q = firing_dict["MS_SSA_Conv" + str(i) + "_q_lif"]
            q = (
                q.transpose(-2, -1)
                .contiguous()
                .reshape(args.time_steps, args.val_batch_size, -1, 14 * 14)
                .reshape(args.time_steps, args.val_batch_size, -1, 14, 14)
            )
            # k = firing_dict["MS_SSA_Conv" + str(i) + "_k_lif"]
            # k = k.transpose(-2, -1).contiguous().reshape(args.time_steps, args.val_batch_size, -1, 14 * 14).reshape(args.time_steps, args.val_batch_size, -1, 14, 14)
            # v = firing_dict["MS_SSA_Conv" + str(i) + "_v_lif"]
            # v = v.transpose(-2, -1).contiguous().reshape(args.time_steps, args.val_batch_size, -1, 14 * 14).reshape(args.time_steps, args.val_batch_size, -1, 14, 14)
            qkv = firing_dict["MS_SSA_Conv" + str(i) + "_x_after_qkv"]
            qkv = (
                qkv.transpose(-2, -1)
                .contiguous()
                .reshape(args.time_steps, args.val_batch_size, -1, 14 * 14)
            ...
            plot_spike(q, filename="images/" + str(index) + "/q" + str(i) + ".png")
            # plot_spike(k, filename="images/"+str(index)+"/k" + str(i) + ".png")
            # plot_spike(v, filename="images/"+str(index)+"/v" + str(i) + ".png")
            # plot_spike(k*v, filename="images/"+str(index)+"/kv" + str(i) + ".png")
            plot_spike(qkv, filename="images/" + str(index) + "/qkv" + str(i) + ".png")

            kv = firing_dict["MS_SSA_Conv" + str(i) + "_kv_before"]
            kv_after = firing_dict["MS_SSA_Conv" + str(i) + "_kv"]
            kv_after = (
                kv_after.detach()
                .transpose(-1, -2)
                .reshape(args.time_steps, args.val_batch_size, -1, 1, 1)
            )
            kv_grad = torch.autograd.grad(loss, kv, retain_graph=True)[0]
            # kv = (kv - kv.min()) / (kv.max() - kv.min())
            kv = kv_grad.sigmoid() * kv.detach()
            # kv = kv.detach()
            kv = (
                kv.transpose(-2, -1)
                .contiguous()
            ...
            kv = kv.mean(dim=0)
            kv_after = kv_after.mean(dim=0)
            # kv = kv[0, ...]
            # kv_after = kv_after[0, ...]

            kv = torch.nn.Upsample(scale_factor=16)(kv) * kv_after
            kvs.append(kv)

        kv_img = sum(kvs).mean(dim=1).squeeze(0).cpu().detach().numpy()
        kv_img_lif = (
            MultiStepLIFNode(v_threshold=sum(kvs).mean().item())(sum(kvs).mean(dim=1))
            .cpu()
            .detach()
            .numpy()
        )

        kv_img = kv_img / (kv_img.max() - kv_img.min() + 1e-10)
        vis_saliency_kde(
            kv_img, input_img, filename="images/" + str(index) + "/out.png"
        )

        kv_img = kv_img_lif[0, ..., np.newaxis]

        # kv_img = (kv_img - kv_img.min()) / (kv_img.max() - kv_img.min())
        kv_img = input_img * kv_img
        kv_img = cv2.cvtColor(kv_img, cv2.COLOR_RGB2BGR)
        cv2.imwrite("images/" + str(index) + "/out2.png", kv_img.astype(np.uint8))
