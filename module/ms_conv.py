...
class MS_SSA_Conv(nn.Module):
    def __init__(
        ...
        if (
            mode == "hydra_lif"
            or mode == "hydra_direct_lif"
            or mode == "hydra_direct_lif_wov"
        ):
    ...
    def forward(self, x, hook=None):
        ...
        if self.mode == "hydra_direct_lif_wov":
            pass
        else:
        ...
        if self.mode == "direct_xor":
            attn = (1 - q).mul(1 - k) + q.mul(k)
            x = attn.mul(v)
        elif self.mode == "hydra":
            kv = k.mul(v).sum(dim=-2, keepdim=True)
            x = q.mul(kv)
        elif self.mode == "hydra_direct_lif" or self.mode == "hydra_direct_lif_wov":
            kv = k.mul(v)
            if hook is not None:
...
class Channel_SSA_Conv(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=8,
...
class Spatial_SSA_Conv(Channel_SSA_Conv):
    def __init__(
        self,
        dim,
        num_heads=8,
...
class Hybrid_Block_Conv(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
        ...
        super().__init__()
        # self.norm1 = norm_layer(dim)
        self.channel_attn = Channel_SSA_Conv(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        # self.norm2 = norm_layer(dim)
        self.spatial_attn = Spatial_SSA_Conv(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
